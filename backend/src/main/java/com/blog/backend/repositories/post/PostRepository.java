 ...
 * all you have to do is define method signatures following spring data jpa naming conventions.
 * for example the findByUser method above will generate a query that finds all posts
...
Example:
findTop10ByUserAndTitleContainingIgnoreCaseOrderByCreatedAtDesc(User user, String keyword)

Explanation:
- Top10                  -> limit 10 results
- ByUser                  -> filter by user
- AndTitleContainingIgnoreCase -> filter title containing keyword ignoring case
- OrderByCreatedAtDesc    -> sort by creation date descending

7️⃣ Notes / Tips
------------------------------------------------
- CamelCase matters: CreatedAt must match the entity field.
- Keyword order matters: findByTitleContainingAndUser works, findByAndUserTitleContaining does not.
- Boolean fields: use IsTrue / IsFalse
  e.g., findByPublishedIsTrue()
- Relationships: you can traverse nested objects
  e.g., findByUserEmail(String email) // Post has User user, User has String email
